...
def print_calc_st_res(f, n_st):
    ...
    curr_str += "|  " + to_fixed(f.v["pt2alpha_3z" + s(n_st)], n_sym)
    curr_str += "|       " + to_fixed(f.v["pt2cos_alpha_3z" + s(n_st)], n_sym)
    if f.v["cz*" + s(n_st)] < 100.0:
        curr_str += "|  " + to_fixed(f.v["cz*" + s(n_st)], n_sym)
    else:
        curr_str += "| " + to_fixed(f.v["cz*" + s(n_st)], n_sym)
    if f.v["pt2c1*" + s(n_st)] < 100.0:
        curr_str += "|  " + to_fixed(f.v["pt2c1*" + s(n_st)], n_sym)
    else:
        curr_str += "| " + to_fixed(f.v["pt2c1*" + s(n_st)], n_sym)
...
def print_calc_la_res(f, n_st):
    n_sym = 3
    curr_str = ""
    curr_str += "|ЛА"
    if f.v["p_з*" + s(n_st - 1)] < 100000.0:
        curr_str += "|  " + to_fixed(f.v["p_з*" + s(n_st - 1)], n_sym)
    else:
        curr_str += "| " + to_fixed(f.v["p_з*" + s(n_st - 1)], n_sym)
    curr_str += "| " + to_fixed(f.v["T_з*" + s(n_st - 1)], n_sym)
    curr_str += "| " + to_fixed(f.v["D_вт" + s(n_st - 1)], n_sym)
    curr_str += "| " + to_fixed(f.v["D_н" + s(n_st - 1)], n_sym)
    curr_str += "| " + to_fixed(f.v["F_" + s(n_st - 1)], n_sym)
    curr_str += "| " + to_fixed(f.v["pt1D_ср" + s(n_st)], n_sym)
    curr_str += "| " + to_fixed(f.v["D_р" + s(n_st - 1)], n_sym)
    curr_str += "|   " + to_fixed(f.v["pt2r_ср_rel" + s(n_st)], n_sym)
    curr_str += "|  " + to_fixed(f.v["pt2alpha_3z" + s(n_st)], n_sym)
    curr_str += "|       " + to_fixed(f.v["pt2cos_alpha_3z" + s(n_st)], n_sym)
    if f.v["cz*" + s(n_st)] < 100.0:
...
def print_start_str():
    print("*********************************")
    print("calculation of static pressures and temperatures in front of the impeller (second method) (018):")
    curr_str = ""
    curr_str += "|№ "
    curr_str += "|p*         "
    curr_str += "|T*      "
...
def calc_part(f, is_print_res):
    for i in range(1, f.v["i_int"] + 2):
        if i == f.v["i_int"] + 1:
            f.v["pt2D_ср" + s(i)] = (f.v["D_вт" + s(i - 1)] + f.v["D_н" + s(i - 1)]) / 2.0
            f.v["pt2r_ср_rel" + s(i)] = f.v["pt2D_ср" + s(i - 1)] / f.v["D_р" + s(i - 1)]
        else:
            f.v["pt2D_ср" + s(i)] = (f.v["D_вт" + s(i)] + f.v["D_н" + s(i)]) / 2.0
        ...
        f.v["pt2cos_alpha_3z" + s(i)] = math.cos(math.radians(f.v["pt2alpha_3z" + s(i)]))
        if i == f.v["i_int"] + 1:
            rho1 = f.v["p_з*" + s(i - 1)] / (f.v["R"] * f.v["T_з*" + s(i - 1)])
            f.v["cz*" + s(i)] = f.v["m"] / (rho1 * f.v["F_" + s(i - 1)])
        f.v["pt2c1*" + s(i)] = f.v["cz*" + s(i)] / f.v["pt2cos_alpha_3z" + s(i)]
        if i == f.v["i_int"] + 1:
            f.v["pt2T" + s(i)] = f.v["T_з*" + s(i - 1)] - math.pow(f.v["pt2c1*" + s(i)], 2.0) / (2.0 * 1005.0)
        else:
            f.v["pt2T" + s(i)] = f.v["T*" + s(i)] - math.pow(f.v["pt2c1*" + s(i)], 2.0) / (2.0 * 1005.0)
        h1 = f.v["k"] / (f.v["k"] - 1.0)
        if i == f.v["i_int"] + 1:
            f.v["pt2p" + s(i)] = f.v["p_з*" + s(i - 1)] * math.pow(f.v["pt2T" + s(i)] / f.v["T_з*" + s(i - 1)], h1)
        else:
            f.v["pt2p" + s(i)] = f.v["p*" + s(i)] * math.pow(f.v["pt2T" + s(i)] / f.v["T*" + s(i)], h1)
        f.v["pt2rho" + s(i)] = f.v["pt2p" + s(i)] / (f.v["R"] * f.v["pt2T" + s(i)])
        if i == f.v["i_int"] + 1:
            f.v["pt2cz" + s(i)] = f.v["m"] / (f.v["pt2rho" + s(i)] * f.v["F_" + s(i - 1)])
        else:
            f.v["pt2cz" + s(i)] = f.v["m"] / (f.v["pt2rho" + s(i)] * f.v["F_" + s(i)])
        f.v["pt2c1" + s(i)] = f.v["pt2cz" + s(i)] / f.v["pt2cos_alpha_3z" + s(i)]
        if i == f.v["i_int"] + 1:
            f.v["pt2T_res" + s(i)] = f.v["T_з*" + s(i - 1)] - math.pow(f.v["pt2c1" + s(i)], 2.0) / (2.0 * 1005.0)
        else:
            f.v["pt2T_res" + s(i)] = f.v["T*" + s(i)] - math.pow(f.v["pt2c1" + s(i)], 2.0) / (2.0 * 1005.0)
        if i == f.v["i_int"] + 1:
            f.v["pt2p_res" + s(i)] = f.v["p_з*" + s(i - 1)] * math.pow(f.v["pt2T" + s(i)] / f.v["T_з*" + s(i - 1)], h1)
        else:
            f.v["pt2p_res" + s(i)] = f.v["p*" + s(i)] * math.pow(f.v["pt2T" + s(i)] / f.v["T*" + s(i)], h1)
        if is_print_res is True:
            if i == f.v["i_int"] + 1:
                print_calc_la_res(f, i)
            else:
