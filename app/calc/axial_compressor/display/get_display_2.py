...
def param_table_first_st(f):
    ...
    for i in range(1, f.v["i_int"] + 1):
        ...
        str_replace += add_str_table_var(f, "Q", i)
        ...
        str_replace += add_str_table_var(f, "F", i)
...
def sub(data, calc_name, f, is_save):
    ...
    while True:
        ...
        if (f_start != -1) and (f_end != -1):
            ...
            elif param == "table_first_st":
                data = data.replace(data[f_start:f_end], param_table_first_st(f))
        else:
            break
    return data
