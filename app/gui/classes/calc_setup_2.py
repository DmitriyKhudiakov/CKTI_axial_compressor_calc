...
import app.calc.axial_compressor.calc_2 as clc
import app.calc.axial_compressor.calc_run as clc_run
import app.calc.axial_compressor.elem_dict_2 as eld
...
class CalcSetup2:
    def __init__(self, parent_splitter, calc_name, calc_display):
        ...
        self.lbl1 = QLabel("Номер варианта вариантного расчета из предыдущего пункта", self.enter_box)
    ...
    def init_data_enter(self):
        ...
        self.enter_box_h_box_list[-1].addWidget(self.combo)
    ...
    def tap_run_btn(self):
        if self.combo.currentText().isnumeric() is True:
            def threading_func():
                self.calc.calc(self.f, self.is_init_vars, self.calc_name, int(self.combo.currentText()))
                self.calc_part3.calc_setup.init_enter_data(self.f)
                self.calc_part3.calc_setup.f = self.f
                self.is_empty = False
