...
import app.calc.axial_compressor.display.get_display_4 as gd
...
class CalcDisplay4:
    ...
    def desc_list_res(self, desc_elem_dict, f, type_calc, nv):
        for key, value in desc_elem_dict.items():
            if key == "html":
                self.te.setHtml(self.get_display.get_display(self.calc_name, f, "display_4" + str(type_calc) + ".html",
                                                             nv))
                self.te.setMinimumHeight(600)

    def show_res(self, f, desc_elem_dict, type_calc, nv):
        self.desc_list_res(desc_elem_dict, f, type_calc, nv)
        self.te.setMinimumHeight(600)
        self.te.setMinimumWidth(1250)

    def get_html_data_file(self, f, desc_elem_dict, is_empty, nv):
        for key, value in desc_elem_dict.items():
            if key == "html":
                return self.get_display.get_data(self.calc_name, f, value, is_empty, nv)
